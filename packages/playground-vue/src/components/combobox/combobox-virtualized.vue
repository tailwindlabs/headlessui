    ...
    let timezones = computed(() => {
      return query.value === ''
        ...
        : allTimezones.filter((timezone) =>
            timezone.toLowerCase().includes(query.value.toLowerCase())
          )
    })

    return { query, activeTimezone, timezones }
  },
  template: html`
      ...
      <div class="mx-auto w-full max-w-xs">
        ...
        <div class="space-y-1">
          <Combobox v-model="activeTimezone" as="div" :virtual="virtual">
            ...
            <div class="relative">
              <span
                class="relative inline-flex flex-row overflow-hidden rounded-md border shadow-sm"
              >
                <ComboboxInput
