  ...
  <div class="flex h-full w-screen items-start justify-center bg-gray-50 p-12">
    <SwitchGroup as="div" class="flex items-center space-x-4">
      ...
      <Switch
        as="button"
        v-model="state"
        :class="resolveSwitchClass({ checked: state, disabled: disabledToggle })"
        :disable-toggle="disabledToggle"
        v-slot="{ checked }"
      >
        <span
...
export default {
  ...
  setup() {
    let state = ref(false)
    let disabledToggle = ref(true)

    return {
      state,
      disabledToggle,
      resolveSwitchClass({ checked, disabled }) {
        return classNames(
