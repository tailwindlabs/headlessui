        ...
        let api = {
            ...
            search(value: string) {
                ...
                let match = options.value.findIndex(
                    option => !option.dataRef.disabled && option.dataRef.textValue.startsWith(searchQuery.value),
                )
...
export let ListboxButton = defineComponent({
    ...
    render() {
        ...
        let propsWeControl = {
            ...
            'aria-labelledby': api.labelRef.value ? [dom(api.labelRef)?.id, this.id].join(' ') : undefined,
    ...
    render() {
        ...
        let propsWeControl = {
            'aria-activedescendant':
                api.activeOptionIndex.value === null ? undefined : api.options.value[api.activeOptionIndex.value]?.id,
