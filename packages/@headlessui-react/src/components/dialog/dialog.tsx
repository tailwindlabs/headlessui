...
function useScrollLock(
  ...
  useDocumentOverflowLockedEffect(ownerDocument, enabled, [handleIOSScrollLock])

  function handleIOSScrollLock(
    req: ScrollLockRequestWithDisposables,
    next: (req: ScrollLockRequestWithDisposables) => void
  ) {
    if (!isIOS()) {
      return next(req)
    }
    ...
    d.style(ownerDocument.body, 'marginTop', `-${scrollPosition}px`)
    window.scrollTo(0, 0)
