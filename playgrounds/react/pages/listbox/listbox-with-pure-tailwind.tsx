      ...
      <div className="mx-auto w-full max-w-xs">
        <div className="space-y-1">
          <Listbox value={active} onChange={setActivePerson}>
            <Label className="block text-sm leading-5 font-medium text-gray-700">Assigned to</Label>

            <div className="relative">
              <span className="inline-block w-full rounded-md shadow-xs">
                <ListboxButton className="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pr-10 pl-3 text-left transition duration-150 ease-in-out sm:text-sm sm:leading-5">
              ...
              >
                {people.map((name) => (
                  <ListboxOption
                    key={name}
                    value={name}
                    className="group relative cursor-default py-2 pr-9 pl-3 text-gray-900 select-none focus:outline-hidden data-active:bg-indigo-600 data-active:text-white"
                  >
                    <span className="block truncate font-normal group-data-selected:font-semibold">
                      {name}
                    </span>
                    <span className="absolute inset-y-0 right-0 hidden items-center pr-4 text-indigo-600 group-data-active:text-white group-data-selected:flex">
